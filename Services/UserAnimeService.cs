...
namespace AnimeNewsletter.Services
{
    ...
    public class UserAnimeService : IUserAnimeService
    {
        ...
        /// </summary>
        public async Task<IEnumerable<UserAnime>> AddAnimeInBulkAsync(string userEmail, IEnumerable<Anime> anime)
        {
            ...
            var animeList = anime.ToList();

            foreach (var animeItem in animeList)
            {
                // Check if anime exists in the database
                var existingAnime = await _context.Anime
                    .FirstOrDefaultAsync(a => a.Id == animeItem.Id);

                // If it doesn't exist, add it
                if (existingAnime == null)
                {
                    _context.Anime.Add(animeItem);
                    existingAnime = animeItem;
                }

                // Check if the UserAnime entry already exists
                var existingUserAnime = await _context.UserAnime
                    .FirstOrDefaultAsync(ua => ua.UserEmail == userEmail && ua.AnimeId == animeItem.Id);
